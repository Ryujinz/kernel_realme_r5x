...
git clone --depth=1 https://github.com/Ryujinz/Ryujinz-Clang clang

# Main
KERNEL_NAME=Phantom✨ # IMPORTANT ! Declare your kernel name
...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$token/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Compile took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>Realme 5 Series (RMX1911/RMX2030)</b> | <b>$(${CLANG_ROOTDIR}/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')</b>"
